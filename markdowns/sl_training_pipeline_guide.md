...
- it can run all exported feature-set CSVs automatically in batch mode using a manifest file
...
There are now two main ways to use the file:

1. single-dataset mode
2. batch mode over all exported feature datasets

## Workflow

1. Load one feature CSV.
...
In batch mode, this same workflow is repeated once per exported feature-set CSV listed in the manifest.

---

## Input Modes

### 1. Default full-feature dataset

If you run the script without extra arguments, it uses the default path:

- `F:/01_Univalle/01_TG/dataset_features/shallow_learning_birds.csv`

That is the full feature dataset produced by the feature-extraction stage.
...
all train on the same selected feature set.

### 3. Batch mode over all exported datasets

If you want the full feature-family by model comparison, run:

```powershell
python sl_training_pipeline.py --run-exported-batch
```

This uses the default manifest path:

- `F:/01_Univalle/01_TG/sl_results/training_ready_datasets/training_ready_dataset_manifest.csv`

You can also provide a custom manifest:

```powershell
python sl_training_pipeline.py --batch-manifest-csv "F:/01_Univalle/01_TG/sl_results/training_ready_datasets/training_ready_dataset_manifest.csv"
```
...
- `DEFAULT_DATA_CSV_PATH = F:/01_Univalle/01_TG/dataset_features/shallow_learning_birds.csv`
- `DEFAULT_OUT_DIR = F:/01_Univalle/01_TG/sl_outputs`
- `DEFAULT_TRAINING_READY_MANIFEST_PATH = F:/01_Univalle/01_TG/sl_results/training_ready_datasets/training_ready_dataset_manifest.csv`
- `DEFAULT_BATCH_OUT_DIR = F:/01_Univalle/01_TG/sl_outputs_batch`
- `SEED = 42`
- `CV_FOLDS = 5`
- `SCORING = "f1_macro"`
- `VAL_FRACTION = 0.2`
...
When batch mode is used, the file:

1. loads the manifest generated by `sl_feature_comparison.py`
2. iterates over every exported training-ready CSV
3. runs the full single-dataset workflow once per CSV
4. stores each run under a batch folder
5. writes aggregated validation and test comparison tables across all feature families
...
the code writes under:

- `F:/01_Univalle/01_TG/sl_outputs_batch`

Inside it, each batch creates:

- `latest_batch_run.txt`
- `batch_YYYYMMDD_HHMMSS/`

Inside each batch folder, the code writes:

- `per_dataset/`
- `batch_run_manifest.csv`
- `batch_internal_validation_comparison.csv`
- `batch_test_comparison.csv`
- `batch_runtime_summary.json`

The `per_dataset/` folder contains the ordinary per-run outputs for each exported feature-set CSV.

The two most useful aggregated files are:

- `batch_internal_validation_comparison.csv`
- `batch_test_comparison.csv`

These let you compare model performance across both:
...
### 6. Batch mode is the easiest full comparison path

If you want to answer:

- which feature family works best?
- which top-k variant works best?
- which classifier works best on each feature family?

then the simplest workflow is:

1. run `sl_feature_comparison.py`
2. run `python sl_training_pipeline.py --run-exported-batch`

This removes the need to launch one training run manually per exported CSV.

### 7. Time counting and progress bars are different things
...
- feature CSV in `dataset_features` or reduced CSV from `sl_results/training_ready_datasets`
...
- optional batch-run output folders under `sl_outputs_batch`
