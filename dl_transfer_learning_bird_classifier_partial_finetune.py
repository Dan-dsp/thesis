...
3) Trains ONE final deployable model on TRAIN:
      ...
      - warmup head-only then finetune the head + last X internal layers
...
def fine_tune_last_internal_layers(
    model: torch.nn.Module,
    num_internal_layers_to_finetune: int,
) -> List[str]:
    """
    Freeze the full model, then unfreeze:
    - the final classifier layer (model.fc)
    - the last X trainable internal layers before the classifier

    In ResNet-18, "internal layers" here means leaf modules with their own
    parameters, such as Conv2d and BatchNorm2d modules inside layer4/layer3.

    Args:
        model: ResNet-style model with a final classifier named "fc".
        num_internal_layers_to_finetune: X. Use 0 to train only the final layer.

    Returns:
        Names of the internal modules unfrozen for fine-tuning.
    """
    if num_internal_layers_to_finetune < 0:
        raise ValueError("num_internal_layers_to_finetune must be >= 0")
    ...
    selected_layers = internal_layers[-num_internal_layers_to_finetune:] if num_internal_layers_to_finetune else []

    for _, module in selected_layers:
...
def train_final_model_with_internal_val(
    ...
    for epoch in range(1, warmup_epochs + 1):
        ...
        if va_acc > best_val_acc:
            ...
            torch.save({
                ...
                "history": history,
                "num_internal_layers_to_finetune": num_internal_layers_to_finetune,
                "fine_tuned_internal_layers": [],
            }, best_path)
    ...
    # Fine-tune
    print(
        "\n[FINAL TRAIN] Fine-tune: classifier head + "
        f"last {num_internal_layers_to_finetune} internal trainable layer(s)..."
    )
    selected_layers = fine_tune_last_internal_layers(model, num_internal_layers_to_finetune)
    print(f"[FINAL TRAIN] Internal layers unfrozen: {selected_layers if selected_layers else 'none'}")
    print(f"[FINAL TRAIN] Trainable parameters: {count_trainable_parameters(model):,}")
    opt_full = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr_finetune)
    ...
    for epoch in range(1, finetune_epochs + 1):
        ...
        if va_acc > best_val_acc:
            ...
            torch.save({
                ...
                "history": history,
                "num_internal_layers_to_finetune": num_internal_layers_to_finetune,
                "fine_tuned_internal_layers": selected_layers,
            }, best_path)
...
def run_kfold_on_train(
    ...
    for fold_i, (tr_idx_local, va_idx_local) in enumerate(
        ...
        for epoch in range(1, warmup_epochs + 1):
            ...
            if va_acc > best_val_acc:
                ...
                torch.save({"model_state_dict": model.state_dict(),
                            ...
                            "fold": fold_i,
                            "num_internal_layers_to_finetune": num_internal_layers_to_finetune,
                            "fine_tuned_internal_layers": []}, best_path)
        ...
        selected_layers = fine_tune_last_internal_layers(model, num_internal_layers_to_finetune)
        print(
            "[K-FOLD] Fine-tune: classifier head + "
            f"last {num_internal_layers_to_finetune} internal trainable layer(s)"
        )
        print(f"[K-FOLD] Internal layers unfrozen: {selected_layers if selected_layers else 'none'}")
        print(f"[K-FOLD] Trainable parameters: {count_trainable_parameters(model):,}")
        opt_full = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr_finetune)
        for epoch in range(1, finetune_epochs + 1):
            ...
            if va_acc > best_val_acc:
                ...
                torch.save({"model_state_dict": model.state_dict(),
                            ...
                            "fold": fold_i,
                            "num_internal_layers_to_finetune": num_internal_layers_to_finetune,
                            "fine_tuned_internal_layers": selected_layers}, best_path)
...
def main():
    ...
    OUT_DIR = Path("outputs_resnet18_thesis_partial_finetune")
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    ...
    # X: number of last trainable internal layers to unfreeze before model.fc.
    # Use 0 for classifier-head-only fine-tuning.
    num_internal_layers_to_finetune = 4
